...
private:
    ...
    uint32_t speed{5};           //!< Node speed.
    uint32_t nodeCount{20};      //!< Number of nodes.
    uint32_t pktPERsec{100};     //!< Packets per second.

    std::string m_CSVfileName{"rmanet-routing.output.csv"}; //!< CSV filename.
...
void RoutingExperiment::Run()
{
    ...
    int nWifis = nodeCount;

    double TotalTime = 200.0;
    uint32_t rate_calc = 512 * pktPERsec;
    std::string rate = std::to_string(rate_calc) + "bps";
...
void RoutingExperiment::GenCSV()
{
    flowmon->CheckForLostPackets();
    // Ptr<Ipv4FlowClassifier> ipf_classifier = DynamicCast<Ipv4FlowClassifier>(flowmonHelper.GetClassifier());
    std::map<FlowId, FlowMonitor::FlowStats> stats = flowmon->GetFlowStats();

    long double total_throughput = 0.0, total_delay = 0.0;
    int total_rx_pkt = 0, total_tx_pkt = 0, total_lost_pkt = 0;

    for (auto i = stats.begin(); i != stats.end(); ++i)
    {
        // int flow_id = i->first;
        FlowMonitor::FlowStats flow_stats = i->second;
        // Ipv4FlowClassifier::FiveTuple tup = ipf_classifier->FindFlow(flow_id);
        long double time_diff = flow_stats.timeLastRxPacket.GetSeconds() - flow_stats.timeFirstTxPacket.GetSeconds();
        total_throughput += (flow_stats.rxBytes * 8.0) / (time_diff * 1024); // in Kbps
        total_delay += flow_stats.delaySum.GetSeconds();
        total_rx_pkt += flow_stats.rxPackets;
        total_tx_pkt += flow_stats.txPackets;
        total_lost_pkt += flow_stats.lostPackets;
    }

    long double avgThroughput = total_throughput / stats.size();
    long double avgDelay = total_delay / total_rx_pkt;
    long double packetDeliveryRatio = (long double)total_rx_pkt / total_tx_pkt * 100;
    long double packetDropRatio = (long double)total_lost_pkt / total_tx_pkt * 100;

    std::ofstream out("2005014_raodv_sim.csv", std::ios::app);
    out << nodeCount << "," << pktPERsec << "," << speed << "," << avgThroughput << "," << avgDelay << "," << packetDeliveryRatio << "," << packetDropRatio << std::endl;
    out.close();
}