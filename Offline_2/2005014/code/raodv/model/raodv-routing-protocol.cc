    ...
    namespace raodv
    {
        ...
        RoutingProtocol::SendReply(const RreqHeader &rreqHeader, const RoutingTableEntry &toOrigin)
        {
            ...
            if (!rreqHeader.GetUnknownSeqno() && (rreqHeader.GetDstSeqno() == m_seqNo + 1))
                m_seqNo++;
            RevRreqHeader rreq_header(0, 0, 0, m_requestId, rreqHeader.GetDst(), m_seqNo, origin);
            if (m_gratuitousReply)
                rreq_header.SetGratuitousRrep(true);
            if (m_destinationOnly)
                rreq_header.SetDestinationOnly(true);

            rreq_header.SetUnknownSeqno(false);
            rreq_header.SetLifeTime(m_myRouteTimeout);

            // Broadcast RREP instead of unicast
            for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
            {
                Ptr<Socket> socket = j->first;
                Ipv4InterfaceAddress iface = j->second;
                ...
                Ptr<Packet> packet = Create<Packet>();
                rreq_header.SetDst(iface.GetLocal());
                m_rreqIdCache.IsDuplicate(iface.GetLocal(), m_requestId);
        ...
        {
            RevRreqHeader rreq_header;
            p->RemoveHeader(rreq_header);
            Ipv4Address dst = rreq_header.GetDst();

            uint8_t hop = rreq_header.GetHopCount() + 1;
            rreq_header.SetHopCount(hop);

            if (dst == rreq_header.GetOrigin())
                return;

            uint32_t id = rreq_header.GetId();
            if (m_rreqIdCache.IsDuplicate(dst, id))
                return;

            // Create or update forward route entry
            Ptr<NetDevice> dev = m_ipv4->GetNetDevice(m_ipv4->GetInterfaceForAddress(receiver));
            RoutingTableEntry newEntry(dev,
                                       dst,
                                       true,
                                       rreq_header.GetDstSeqno(),
                                       m_ipv4->GetAddress(m_ipv4->GetInterfaceForAddress(receiver), 0),
                                       hop,
                                       sender,
                                       rreq_header.GetLifeTime());

            RoutingTableEntry toDst;
            if (m_routingTable.LookupRoute(dst, toDst))
            {
                if ((!toDst.GetValidSeqNo()) ||
                    ((int32_t(rreq_header.GetDstSeqno()) - int32_t(toDst.GetSeqNo())) > 0) ||
                    (rreq_header.GetDstSeqno() == toDst.GetSeqNo() && toDst.GetFlag() != VALID) ||
                    (rreq_header.GetDstSeqno() == toDst.GetSeqNo() && hop < toDst.GetHop()))
                {
                    m_routingTable.Update(newEntry);
                }
            }
            ...
            if (IsMyOwnAddress(rreq_header.GetOrigin()))
            {
            ...
            RoutingTableEntry toOrigin;
            if (!m_routingTable.LookupRoute(rreq_header.GetOrigin(), toOrigin) || toOrigin.GetFlag() == IN_SEARCH)
                return;
            else if (toOrigin.GetNextHop() == sender)
                return;
